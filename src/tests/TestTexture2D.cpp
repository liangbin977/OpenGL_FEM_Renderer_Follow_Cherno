...
namespace test {
    ...
    {
        float positionsp[] = {
            //对于texture来说，纹理坐标的范围是[0,1]，而不是[-1,1]。这是因为纹理坐标表示的是纹理图像上的位置，而不是屏幕上的位置。
            //如果纹理坐标设置为2，就表示纹理图像上的位置是原点的两倍，也就是纹理图像的右边界之外了。这样会导致纹理采样器根据设置的纹理环绕方式来处理这个坐标，可能会重复、镜像或者使用边缘颜色等。
            -0.5f, -0.5f, 0, 0,//0
             0.5f, -0.5f, 1, 0,//1
             0.5f,  0.5f, 1, 1,//2
            -0.5f,  0.5f, 0, 1//3
